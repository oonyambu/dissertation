...
do.call(rbind, d$lhd_1024) %>%
    ...
    rename("$\\phi(\\cdot)$" = phi)%>%
    kableExtra::kbl(format = 'latex',digits =  c(1,2,2,2,2,4), label='lhd_optim', escape = FALSE,
    caption='Optimal hyperparameter settings for each target design with $4^5$ LHD')

do.call(rbind, d$full_1024)%>%
    `rownames<-` (sub("(\\d+)x(\\d+)", "$\\1\\\\times\\2$", rownames(.)))%>%
    rename("$\\phi(\\cdot)$" = phi)%>%
    kableExtra::kbl(format = 'latex',digits = c(1,2,2,2,2,4), label='grid_optim', escape = FALSE,
    caption='Optimal hyperparameter settings for each target design with $4^5$ FFD')


# b%>%
#    sapply(sapply, getElement,'phi')%>%
#    `colnames<-` (sub("(\\d+)x(\\d+)", "$\\1\\\\times\\2$", colnames(.)))%>%
#    `rownames<-`(sub("(.*)_(\\d+)", "\\2 \\U\\1", perl = TRUE,
#                     basename(rownames(.)))%>%
#                     str_replace_all(c("1024"="$4^5$", "243"="$3^5$", FULL="FFD")))%>%
#    t()%>%
#    kableExtra::kable('latex', escape = FALSE, label='grid_random',
#        caption = 'Average of 10 $\\phi(\\cdot)$ values from the optimal grid and random search methods')
@
...
t(mapply(get_par, c('30x3'=30,'50x5'=50,'70x7'=70), c(3,5,7)))%>%
     `rownames<-` (sub("(\\d+)x(\\d+)", "$\\1\\\\times\\2$", rownames(.)))%>%
    kableExtra::kbl(format = 'latex',digits = 2, escape=FALSE, label='rso_optim',
