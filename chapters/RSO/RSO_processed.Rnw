...
phi_vals_mean <- mapply(\(x,y)colMeans(get_data(x, y, 100)), c(30,50,70), c(3,5,7))
phi_vals_median <- mapply(\(x,y)apply(get_data(x, y, 100), 2, median), c(30,50,70), c(3,5,7))
...
do.call(rbind, d$lhd_1024) %>%
    ...
    mutate(phi = phi_vals_mean['Random', ], phi2 = phi_vals_median['Random',]) %>%
    rename("Average $\\phi(\\cdot)$" = phi, "Median $\\phi(\\cdot)$"=phi2)%>%
    kableExtra::kbl(format = 'latex',digits =  c(0,0,2,2,2,4,4), label='lhd_optim', escape = FALSE,
    caption='Optimal hyperparameter settings for each target design with 1024-run LHD')

do.call(rbind, d$full_1024)%>%
    `rownames<-` (sub("(\\d+)x(\\d+)", "$\\1\\\\times\\2$", rownames(.)))%>%
    mutate(phi = phi_vals_mean['Grid', ],phi2 = phi_vals_median['Grid',]) %>%
    rename("Average $\\phi(\\cdot)$" = phi, "Median $\\phi(\\cdot)$"=phi2) %>%
    kableExtra::kbl(format = 'latex',digits = c(0,0,2,2,2,4, 4), label='grid_optim', escape = FALSE,
...
t(mapply(get_par, c('30x3'=30,'50x5'=50,'70x7'=70), c(3,5,7)))%>%
  ...
  cbind("Average $\\phi(\\cdot)$" = phi_vals_mean['RSO', ], "Median $\\phi(\\cdot)$"=phi_vals_median['RSO',])%>%
    kableExtra::kbl(format = 'latex',digits = c(0,0,2,2,2,4, 4), escape=FALSE, label='rso_optim',
