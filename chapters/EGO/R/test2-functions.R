## test2-fuctions.R - called by km2-simuation.R
## modified from test-fuctions.R, 7/10/24
## 
## R code for JASA paper "A Projection Space-Filling Criterion and Related Optimality Results"
## a collection of test functions called by km-simuation.R 
## (authors) 3/11/2023

library(TestFunctions)

Borehole8 <- function(xx)
{ ##  replace log(TestFunctions::borehole(x[1:8])) # as TF_borehole is wrong, 7/10/24
 # http://www.sfu.ca/~ssurjano/
  ...
  y <- frac1 / frac2
  y=log(y) # using log(y) is recommended by Fang et al. (2006)  
  return(y)
}

Rosenbrock4=Picheny4=function(xx)
{ # Picheny, V., Wagner, T., & Ginsbourger, D. (2012). 4-dim function(x): 0<= x_i <= 1
...
### using library(TestFunctions)
Currin2=function(x) TestFunctions::currin1991(x[1:2])
Circuit6=function(x) TestFunctions::OTL_Circuit(x[1:6])
Piston7=function(x) TestFunctions::piston(x[1:7])
# Borehole8=function(x) log(TestFunctions::borehole(x[1:8])) # TF_borehole is wrong 
Gfunction9=function(x) TestFunctions::Gfunction(x[1:9])
Wingweight10 =function(x) TestFunctions::wingweight(x[1:10])
Gfunction31=function(x) TestFunctions::Gfunction(x[1:31])


fun.lib=list(
	list(name="Currin2", dim=2, fun= Currin2),
	list(name="Rosenbrock4", dim=4, fun= Rosenbrock4),
	list(name="Circuit6", dim=6, fun= Circuit6),
	list(name="Piston7", dim=7, fun= Piston7),
	list(name="Borehole8", dim=8, fun=Borehole8),
	list(name="Gfunction9", dim=9, fun= Gfunction9),
	list(name="Wingweight10", dim=10, fun= Wingweight10),
	list(name="OakleyOhagan15", dim=15, fun= OakleyOhagan15),
	list(name="Gfunction31", dim=31, fun= Gfunction31)
)
