...
%On the other hand, when OACD was used as the training dataset with the $30\times3$ UPD as the target dataset, we note that a local minimum is achieved around $pMut \approx \Sexpr{round(a[[c(4,1,4)]],2)}$ with $pCR \approx \Sexpr{round(a[[c(4,1,5)]],2)}$. When the target was $50\times 5$ the local minima was $pMut \approx \Sexpr{round(a[[c(5,1,4)]],2)}$ and $pCR \approx \Sexpr{round(a[[c(5,1,5)]],2)}$ while when the target was $70\times7$ the minima occured at $pMut \approx \Sexpr{round(a[[c(6,1,4)]],2)} $ and $pCR \approx \Sexpr{round(a[[c(6,1,5)]],2)}$.
