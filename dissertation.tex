...
\begin{itemize}
    ...
    \item itermax -  the maximum number of iterations used. % We consider values between $[500, 1500]$
...
% Once the models were fitted on the generated data, they were evaluated by testing them on the $3^5$ full factorial design (FFD), $243$ run Latin Hypercube design, a combination of $3^5$ FFD and $243$ LHD and on a $4^5$ FFD.
...
%-------------------------------------------------------
To have a better understanding of the interactions, we examine some interaction plots from the full $4^5$  factorial, shown in Figure \ref{interaction}. % It is not easy to interpret the interaction plots from the four datasets due to unequal number of replicates  determine the best levels to set for these variables.
...
These results reveal that it is unlike to have a simple generic setting for pMut and pCR because these two parameters depend highly on the training dataset and also on the size of the target design. % It is clear that there does not a simple choice of pCR and pMut to optimize all cases.
A further study focusing on pCR and pMut is needed to determine their optimal settings.
% This contradicts the message given by other two design sizes mentioned. Thus regarding the fair settings of pMut and pCR, its inconclusive.

%On the other hand, when OACD was used as the training dataset with the $30\times3$ UPD as the target dataset, we note that a local minimum is achieved around $pMut \approx 0.55$ with $pCR \approx 0.33$. When the target was $50\times 5$ the local minima was $pMut \approx 0.28$ and $pCR \approx 0.57$ while when the target was $70\times7$ the minima occured at $pMut \approx 0.05 $ and $pCR \approx 0.91$.
...
%\reva{ToDO: Draw the predicted contour plot for pMut and pCR, based on the OACD and the second-order model with NP=100, itermax=1500 and pGBest=0.95. From which, recommend the choice of pMut and pCR. Then compare with the default settings used by \textcite{stokes2023metaheuristic}, which are  pMut=0.1 and pCR=0.5.}
